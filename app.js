...
function renderShop() {
  ...
  Object.values(products).forEach(p => {
    ...
    card.innerHTML = `
      <div class="shop-card-img"><img alt=""/></div>
...
function renderProduct(id) {
  ...
  Object.values(products).filter(rp => String(rp.id) !== String(id)).slice(0,3).forEach(rp => {
    ...
    card.innerHTML = `
      <div class="related-card-img"><img alt=""/></div>
      <div class="name"></div>
      <div class="price"></div>`;
...
async function startApp() {
  ...
  // On initial load, never land on admin pages — always start at home.
  // This prevents bookmarks/shared links/browser history from exposing the admin route.
